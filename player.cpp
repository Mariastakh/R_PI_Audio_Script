...
void play(double *output, double inputBufferValue);//run dac! Very very often. Too often in fact. er...
...
int routing	(void *outputBuffer, void *inputBuffer, unsigned int nBufferFrames,
	...
	double *inBuffer = (double *) inputBuffer; //UNCOMMENT FOR INPUT
#endif
	...
	for ( i=0; i<nBufferFrames; i++ ) {
		play(lastValues, *inBuffer++);//UNCOMMENT FOR INPUT			
		//play(lastValues);
...
int main()
{
	...
	parameters.deviceId = 2; // THE  DEFAULT SCREEN SPEAKERS = 1
	parameters.nChannels =maxiSettings::channels;
	parameters.firstChannel = 0;

	// MY INPUT PARAMS:
	RtAudio::StreamParameters inParameters;
	inParameters.deviceId = 2; // THE FOCUSRITE INTERFACE = 2
	inParameters.nChannels = 1;//maxiSettings::channels; manually set input to mono
	...
	try {
		dac.openStream( &parameters, &inParameters, RTAUDIO_FLOAT64,
